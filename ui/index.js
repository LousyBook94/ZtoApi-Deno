  ...
  card.addEventListener("mousemove", (e) => {
    ...
    if (button) {
      ...
      button.style.setProperty(
        "transform",
        `translateZ(${buttonTranslateZ}px) rotateX(${buttonRotateX}deg) rotateY(${buttonRotateY}deg) scale(1.05)`,
        "important",
      );
      button.style.setProperty(
        "box-shadow",
        `0 ${10 + Math.abs(rotateY)}px ${25 + Math.abs(rotateX)}px rgba(0, 0, 0, 0.4), 0 0 ${
          20 + Math.abs(rotateX) + Math.abs(rotateY)
        }px var(--glow-color)`,
        "important",
      );
    }
