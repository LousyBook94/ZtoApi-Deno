...
function updateStats() {
  fetch("/dashboard/stats")
    ...
    .then((data) => {
      ...
      animateValue(
        "total-requests",
        parseInt(document.getElementById("total-requests").textContent) || 0,
        data.totalRequests || 0,
        1000,
      );
      animateValue(
        "successful-requests",
        parseInt(document.getElementById("successful-requests").textContent) || 0,
        data.successfulRequests || 0,
        1000,
      );
      animateValue(
        "failed-requests",
        parseInt(document.getElementById("failed-requests").textContent) || 0,
...
function updateTable() {
  ...
  currentRequests.forEach((request) => {
    ...
    row.innerHTML = "<td>" + timeStr + "</td>" + "<td>" + modelName + "</td>" + "<td>" +
      (request.method || "undefined") + "</td>" + "<td class='" + statusClass + "'>" + status + "</td>" + "<td>" +
      ((request.duration / 1000).toFixed(2) || "undefined") + "s</td>" + "<td title='" + (request.user_agent || "") +
      "'>" + userAgent + "</td>";
